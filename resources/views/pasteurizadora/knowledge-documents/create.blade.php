...
<div class="mx-auto max-w-5xl space-y-6">
    <div class="rounded-3xl bg-slate-900 px-6 py-6 text-white shadow-xl">
        <a href="{{ route('pasteurizadora.knowledge-documents.index') }}" class="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-semibold text-white/90 transition hover:bg-white/20">
            <i class="fas fa-arrow-left"></i>
            Volver a la base
        </a>
        <h1 class="mt-4 text-3xl font-black tracking-tight">Cargar documento para la IA de Pasteurizadora</h1>
    ...
    <form method="POST" action="{{ route('pasteurizadora.knowledge-documents.store') }}" enctype="multipart/form-data" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        @csrf
        ...
        <div class="grid gap-5 md:grid-cols-2">
            ...
            <div>
                <label for="component_code" class="mb-2 block text-sm font-semibold text-slate-700">Componente</label>
                <select id="component_code" name="component_code" class="w-full rounded-xl border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                    <option value="">Todos</option>
                    @foreach($componentes as $componente)
                        <option value="{{ $componente['code'] }}" @selected(old('component_code') === $componente['code'])>
                            {{ $componente['name'] }} ({{ $componente['code'] }}) - {{ $areas[$componente['area']] ?? $componente['area'] }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                <label for="component_name" class="mb-2 block text-sm font-semibold text-slate-700">Nombre de componente libre</label>
                <input id="component_name" name="component_name" type="text" value="{{ old('component_name') }}" class="w-full rounded-xl border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            </div>
            ...
            <div>
                <label for="modulo" class="mb-2 block text-sm font-semibold text-slate-700">Modulo</label>
                <input id="modulo" name="modulo" type="number" min="1" max="99" value="{{ old('modulo') }}" class="w-full rounded-xl border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            </div>

            <div>
                <label for="nivel" class="mb-2 block text-sm font-semibold text-slate-700">Nivel</label>
                <input id="nivel" name="nivel" type="text" value="{{ old('nivel') }}" class="w-full rounded-xl border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            </div>

            <div>
                <label for="piso" class="mb-2 block text-sm font-semibold text-slate-700">Piso</label>
                <input id="piso" name="piso" type="text" value="{{ old('piso') }}" class="w-full rounded-xl border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            </div>

            <div>
                <label for="lado" class="mb-2 block text-sm font-semibold text-slate-700">Lado</label>
                <input id="lado" name="lado" type="text" value="{{ old('lado') }}" class="w-full rounded-xl border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            </div>
        ...
        <div class="mt-6 flex flex-wrap gap-3">
            ...
            <a href="{{ route('pasteurizadora.knowledge-documents.index') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
