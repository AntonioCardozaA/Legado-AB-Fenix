...
@section('title', 'Base de Conocimiento Pasteurizadora')
...
<div class="mx-auto max-w-7xl space-y-6">
    <div class="rounded-3xl bg-slate-900 px-6 py-6 text-white shadow-xl">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <a href="{{ route('plan-accion.ai.pasteurizadora.index') }}" class="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-semibold text-white/90 transition hover:bg-white/20">
                    <i class="fas fa-arrow-left"></i>
                    Volver a revision IA
                </a>
                <h1 class="mt-4 text-3xl font-black tracking-tight">Base de conocimiento de Pasteurizadora</h1>
                <p class="mt-2 max-w-3xl text-sm text-slate-300">
                    Manuales, procedimientos y referencias historicas que alimentan el contexto del generador de planes.
                </p>
            </div>

            <a href="{{ route('pasteurizadora.knowledge-documents.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-amber-400 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-amber-300">
    ...
    <form method="GET" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="grid gap-4 md:grid-cols-5">
            ...
            <div>
                ...
                <select name="area" class="w-full rounded-xl border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                    <option value="">Todas</option>
            ...
            <div class="flex items-end gap-3">
                ...
                <a href="{{ route('pasteurizadora.knowledge-documents.index') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                    ...
                    @forelse($documents as $document)
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-4 align-top text-slate-600">
                                ...
                                <p class="mt-1 text-xs">{{ $areas[$document->area] ?? 'Todas las areas' }}</p>
                                <p class="mt-1 text-xs">{{ $document->component_name ?: $document->component_code ?: 'Todos los componentes' }}</p>
                                @if($document->modulo || $document->nivel || $document->piso || $document->lado)
                                    <p class="mt-1 text-xs text-slate-500">
                                        {{ implode(' | ', array_filter([
                                            $document->modulo ? 'Modulo '.$document->modulo : null,
                                            $document->nivel ? 'Nivel '.$document->nivel : null,
                                            $document->piso ? 'Piso '.$document->piso : null,
                                            $document->lado ? 'Lado '.$document->lado : null,
                                        ])) }}
                                    </p>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-4 align-top">
                                <form method="POST" action="{{ route('pasteurizadora.knowledge-documents.reindex', ['document' => $document->id]) }}">
                                    @csrf
