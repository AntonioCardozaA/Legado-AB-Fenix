...
<div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        {{-- Formulario --}}
        <form method="POST" action="{{ route('analisis-tendencia-mensual.pasteurizadora.store') }}" class="p-4 sm:p-8">
            @csrf
...
document.getElementById('linea_id').addEventListener('change', function() {
    if (this.value) {
        window.location.href = '{{ route("analisis-tendencia-mensual.pasteurizadora.create") }}?linea_id=' + this.value;
    }
});
</script>
@endsection
