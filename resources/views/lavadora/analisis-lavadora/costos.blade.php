...
<div class="mx-auto max-w-7xl space-y-6">
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-200 bg-white px-4 py-6 sm:px-8 sm:py-7">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div class="min-w-0 space-y-3">
                    <div class="flex flex-wrap items-center gap-2 text-sm text-slate-600">
                        <span class="max-w-full break-words rounded-full border border-slate-200 bg-slate-50 px-3 py-1 font-semibold text-slate-700">Analisis #{{ $analisislavadora->id }}</span>
                        <span class="max-w-full break-words rounded-full border border-slate-200 bg-slate-50 px-3 py-1 font-semibold text-slate-700">{{ $analisislavadora->linea->nombre ?? 'Sin linea' }}</span>
                        <span class="max-w-full break-words rounded-full border border-slate-200 bg-slate-50 px-3 py-1 font-semibold text-slate-700">{{ optional($analisislavadora->fecha_analisis)->format('d/m/Y') ?? 'Sin fecha' }}</span>
                    </div>
                    <div>
                    ...
                    <div class="break-words text-sm text-slate-600">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                    <a href="{{ route('analisis-lavadora.index', array_filter(['linea_id' => $analisislavadora->linea_id], fn ($value) => filled($value))) }}"
                       class="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-slate-300 bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-800 shadow-sm transition hover:bg-slate-200 sm:w-auto">
                        <i class="fas fa-arrow-left"></i>
                        Volver al analisis
                    </a>
                    <a href="{{ route('lavadora.costos.index') }}"
                       class="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-sky-200 bg-sky-100 px-4 py-2.5 text-sm font-semibold text-sky-800 transition hover:bg-sky-200 sm:w-auto">
        ...
        <div class="grid gap-4 border-t border-slate-200 bg-slate-50 px-4 py-5 sm:grid-cols-3 sm:px-8">
    ...
    @if($costEntries->isEmpty())
        <section class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-5 text-sm text-amber-900 shadow-sm sm:px-6">
            <div class="flex items-start gap-3">
                <span class="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-amber-600">
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <div class="mb-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            @if($canEditLavadoraCosts && $canSyncAutomaticCosts)
                <form method="POST" action="{{ route('analisis-lavadora.costos.automatic.sync', ['analisislavadora' => $analisislavadora->id]) }}" class="w-full lg:w-auto">
                    @csrf
        ...
        @if($missingAutomaticEntriesCount > 0)
            <div class="mb-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-900 sm:px-5">
                <div class="flex items-start gap-3">
                    <span class="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-amber-600">
        ...
        @if($automaticSuggestions->isNotEmpty())
            <div class="grid gap-4 lg:grid-cols-2">
                @foreach($automaticSuggestions as $suggestion)
                    <article class="rounded-2xl border {{ $suggestion['excluded'] ? 'border-amber-200 bg-amber-50/70' : 'border-emerald-200 bg-emerald-50/70' }} p-4 shadow-sm sm:p-5">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            <div class="min-w-0 space-y-2">
                                <div class="flex flex-wrap items-center gap-2">
                                    <span class="inline-flex max-w-full items-center break-words rounded-full px-3 py-1 text-xs font-semibold {{ $suggestion['excluded'] ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800' }}">
                                        {{ $suggestion['excluded'] ? 'Desactivada en este analisis' : 'Activa en este analisis' }}
                                    </span>
                                    <span class="inline-flex max-w-full items-center break-words rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
                                        {{ $suggestion['trigger_label'] }}
                                    </span>
                                </div>
                                <div>
                                    <h3 class="break-words text-base font-bold text-slate-900">{{ $costDisplayText($suggestion['catalog_name']) }}</h3>
                                ...
                                <div class="grid gap-2 break-words text-sm text-slate-700 sm:grid-cols-2">
                                ...
                                @if($suggestion['notes'])
                                    <p class="break-words rounded-xl bg-white/80 px-3 py-2 text-sm text-slate-600">{{ $costDisplayText($suggestion['notes']) }}</p>
                                @endif
                            </div>
        ...
        @else
            <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-500 sm:px-6 sm:py-10">
        ...
        @if($costEntries->isNotEmpty())
            <div class="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 md:bg-white">
                <div class="md:overflow-x-auto">
                    <table class="block w-full text-sm md:table md:min-w-[56rem] md:divide-y md:divide-slate-200">
                        ...
                        <tbody class="block space-y-3 bg-slate-50 p-3 md:table-row-group md:space-y-0 md:divide-y md:divide-slate-100 md:bg-white md:p-0">
                            @foreach($costEntries as $entry)
                                <tr class="block rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:table-row md:border-0 md:p-0 md:shadow-none">
                                    <td class="block md:table-cell md:px-4 md:py-4 md:align-top">
                                        <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 md:hidden">Concepto</span>
                                        <div class="break-words font-semibold text-slate-900">{{ $costDisplayText($entry->catalog_name_snapshot) }}</div>
                                    ...
                                    <td class="mt-3 block border-t border-slate-100 pt-3 md:table-cell md:border-0 md:px-4 md:py-4 md:align-top">
                                        <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 md:hidden">Origen</span>
                                        <div class="flex flex-wrap gap-2">
                                            <span class="inline-flex max-w-full break-words rounded-full px-3 py-1 text-xs font-semibold {{ $entry->isManual() ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800' }}">
                                                {{ \App\Models\LavadoraCostEntry::originLabel($entry->source_type) }}
                                            </span>
                                            <span class="inline-flex max-w-full break-words rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
                                                {{ \App\Models\LavadoraCostEntry::sourceLabel($entry->source_type) }}
                                            </span>
                                        </div>
                                        @if($entry->source_reference)
                                            <div class="mt-2 break-words text-xs text-slate-500">Referencia: {{ $costDisplayText($entry->source_reference) }}</div>
                                        @endif
                                    </td>
                                    <td class="mt-3 block md:table-cell md:px-4 md:py-4 md:align-top">
                                        <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 md:hidden">Cantidad</span>
                                        <span class="text-slate-700">{{ number_format((float) $entry->quantity, 2) }} {{ $entry->unidad_medida_snapshot ?: 'unidad' }}</span>
                                    </td>
                                    <td class="mt-3 block md:table-cell md:px-4 md:py-4 md:align-top">
                                        <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 md:hidden">Costo unitario</span>
                                        <span class="text-slate-700">${{ number_format((float) $entry->unit_cost, 2) }}</span>
                                    </td>
                                    <td class="mt-3 block md:table-cell md:px-4 md:py-4 md:align-top">
                                        <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 md:hidden">Total</span>
                                        <span class="font-semibold text-slate-900">${{ number_format((float) $entry->total_cost, 2) }}</span>
                                    </td>
                                    <td class="mt-3 block md:table-cell md:px-4 md:py-4 md:align-top">
                                        <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 md:hidden">Notas</span>
                                        <span class="break-words text-slate-600">{{ $costDisplayText($entry->notas) ?: 'Sin notas' }}</span>
                                    </td>
                                    <td class="mt-4 block md:table-cell md:px-4 md:py-4 md:text-right md:align-top">
                                        <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 md:hidden">Acciones</span>
                                        @if($entry->isManual() && $canDeleteLavadoraCosts)
                                            <form method="POST" action="{{ route('analisis-lavadora.costos.manual.destroy', ['analisislavadora' => $analisislavadora->id, 'costEntry' => $entry->id]) }}" class="w-full md:w-auto">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-red-50 px-3 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-100 md:w-auto">
                                                    <i class="fas fa-trash"></i>
                                                    Quitar
                                                </button>
                                            </form>
                                        @else
                                            <span class="block break-words rounded-lg bg-slate-50 px-3 py-2 text-left text-xs text-slate-500 md:bg-transparent md:p-0 md:text-right md:text-slate-400">Gestiona desde reglas automaticas</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <form method="POST" action="{{ route('analisis-lavadora.costos.manual.store', ['analisislavadora' => $analisislavadora->id]) }}" class="space-y-5">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
                <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-sky-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-sky-700 sm:w-auto">
